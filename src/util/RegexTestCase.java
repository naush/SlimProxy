...
public abstract class RegexTestCase extends TestCase {
	...
	public static void assertHasRegexp(String regexp, String output) {
		Matcher match = Pattern.compile(regexp,
				Pattern.MULTILINE | Pattern.DOTALL).matcher(output);
	...
	public static void assertDoesntHaveRegexp(String regexp, String output) {
		Matcher match = Pattern.compile(regexp, Pattern.MULTILINE).matcher(
				output);
		boolean found = match.find();
		if (found)
	...
	public static String divWithIdAndContent(String id,
			String expectedDivContent) {
